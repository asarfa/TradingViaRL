...
class Agent(metaclass=abc.ABCMeta):
    ...
    def _get_path(self, episode: str = None):
        ...
        agent = os.path.join('agents', 'savings', self.get_name(), ticker)
    ...
    def _delete_earlier_args(self, path, episode, only_models: bool = True):
        if only_models:
            # keep only previous models
            args_file = [d for d in os.listdir(path.replace('Episode' + str(episode), '')) if d.find('model')==-1]
        else:
            # keep only last agent
            args_file = [d for d in os.listdir(path.replace('Episode' + str(episode), '')) if d.find('agent') == -1]
    ...
    def _set_best_ep(self):
        ...
        if self.learning_agent:
            ...
            if not os.path.exists(new_path):
                ...
                self.model.set(path, best_ep)
                self.model.save_args(new_path)
                self._delete_earlier_args(path, None, False)
            else:
                self.model.set(path, os.path.join(new_path, '_model.pkl'))
